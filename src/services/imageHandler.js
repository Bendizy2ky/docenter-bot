const { compressImage, removeBackground, makePassportPhoto, convertImage, applyBackground } = require('../services/image');
  ...
  return {
    ...
    process: async (ctx, tool, fileBuffer, fileName, mimeType, state, extendedShared) => {
      ...
      if (tool === 'apply_background') {
        const bgColor = state.bgColor || 'white'; // Default to white if not set

        // Step 1: Remove background
        const removedBgResult = await removeBackground(fileBuffer);
        if (!removedBgResult.success) {
          throw new Error('Background removal failed for apply background feature.');
        }

        // Step 2: Apply solid color background
        const finalImageResult = await applyBackground(removedBgResult.buffer, bgColor);
        if (!finalImageResult.success) {
          throw new Error('Applying solid background failed.');
        }
